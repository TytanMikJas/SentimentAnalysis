...
def run_test_classifiers(path_to_split_data, metrics_file, params):
    ...
    for vect_name, vect in vectorizers.items():
        log_info(
            f"TESTTING {use_data.upper()} FOR {clf_name.upper()} CLASSIFIER {vect_name.upper()}"
        )
        wandb.init(project="pdiow-lab-vect-test", name=f"{vect_name}", reinit=True)
        ...
        for train_idx, test_idx in skf.split(X, y):
            ...
            pipeline = build_pipeline(
                params, use_data=use_data, classifier=clf, vec_method=vect
            )
        ...
        save_f1_score(
            np.mean(f1_test_scores), f"{clf_name} on {vect_name}", metrics_file
        )
    ...
    with open("data/models/best_vec_method.json", "w") as f:
